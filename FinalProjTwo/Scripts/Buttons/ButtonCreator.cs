namespace DrawingProgram
{
    public static class ButtonCreator
    {
        ...
        "Color picker", "Checker/Dither (Rig)", "Draw rectangle",
        ...
        public static List<IHoverable> GenerateButtons(ProgramManager program, ToolFolder inputTools, Canvas canvas)
        {
            ...
            foreach (var buttonType in ButtonPositions.Keys)
            {
                ...
                {
                    button = (Button)Activator.CreateInstance(buttonType, program, ButtonPositions[buttonType], canvas);
                }
                else button = (Button)Activator.CreateInstance(buttonType, program, ButtonPositions[buttonType]);
